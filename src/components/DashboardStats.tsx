...
import { SalaryAgreement, Employee } from '../types';
...
export const DashboardStats: React.FC<DashboardStatsProps> = ({
  ...
  const totalMonthlyPayroll = activeAgreements.reduce(
    (acc, curr) => acc + (curr.monthlySalary || 0),
    0
  );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">סה״כ הסכמים</span>
          ...
          <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-600">
            <span>{employees.length} עובדים · {totalRolesTagged} תפקידים</span>
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-sky-700 uppercase tracking-wider">ממתינים לחתימה</span>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">היקף שכר חודשי</span>
          ...
          <div className="flex items-baseline justify-between">
            <div className="text-3xl font-extrabold text-slate-900">
              ₪{Math.round(totalMonthlyPayroll / 1000)}k
            </div>
            <span className="text-xs text-slate-500 font-medium">בחודש</span>
          </div>
          <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-600">
            ...
            <span className="font-bold flex items-center" style={{ color: 'var(--accent)' }}>
